...
const apiKey = "4b7a35e0421b25f7db9acada";
...
getRatesBtn.addEventListener("click", () => {
  ...
  fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/latest/${base}`)
    ...
    .then((data) => {
      let rateHtml = "<h2>Exchange Rates</h2><ul>";
      for (const [currency, rate] of Object.entries(data.conversion_rates)) {
        if (currency !== base) {
          rateHtml += `<li class="currency-icon"><span>${currency}</span>${currency} : ${rate.toFixed(
            4
          )}</li>`;
        }
      }
      rateHtml += "</ul>";
      exchangeRates.innerHTML = rateHtml;
    })
    .catch((error) => {
      console.log(error);
    });
